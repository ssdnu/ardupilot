...
static bool guided_ground_vel_override_notified; // true if ground velocity override has been reported
...
{
    // if not armed set throttle to zero and exit immediately unless ground velocity override is enabled
    const bool allow_ground_vel = option_is_enabled(Option::AllowGroundVelocity);
    if (!motors->armed() || (copter.ap.land_complete && !allow_ground_vel)) {
        // do not spool down tradheli when on the ground with motor interlock enabled
        make_safe_ground_handling(copter.is_tradheli() && motors->get_interlock());
        return;
    }

    if (copter.ap.land_complete && allow_ground_vel) {
        if (!guided_ground_vel_override_notified) {
            AP::logger().Write_Message("GUIDED_GNDVEL_OVERRIDE");
            gcs().send_text(MAV_SEVERITY_INFO, "GUIDED: Ground velocity override");
            guided_ground_vel_override_notified = true;
        }
    } else {
        guided_ground_vel_override_notified = false;
    }
